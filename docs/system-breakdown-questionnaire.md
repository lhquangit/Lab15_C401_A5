...
Trả lời đề xuất: MVP tập trung giải quyết 2 vấn đề vận hành nội bộ lớn nhất: giảm thời gian tra cứu quy trình và trả lời câu hỏi hành chính lặp lại; đồng thời giảm thời gian tóm tắt hồ sơ bệnh án cho nhân sự được phân quyền. MVP không nhằm hỗ trợ quyết định lâm sàng.

2. User nào được phục vụ trong MVP?  
Trả lời đề xuất: Bác sĩ, điều dưỡng, và nhân viên tiếp nhận là 3 nhóm user nội bộ được phục vụ trong MVP.

3. User nào không nằm trong MVP?  
Trả lời đề xuất: Bệnh nhân, người nhà bệnh nhân, người dùng bên ngoài bệnh viện, đối tác ngoài hệ thống, và các bộ phận chưa được cấp quyền truy cập dữ liệu trong giai đoạn đầu không nằm trong MVP.

4. 3-5 use case bắt buộc phải có để go-live là gì?  
Trả lời đề xuất:
- Tóm tắt hồ sơ bệnh án theo mẫu chuẩn ở chế độ `read-only`
- Hỏi đáp quy trình nội bộ dựa trên tài liệu chính thức
- Hỏi đáp hành chính như lịch khám, thủ tục, biểu mẫu
- Phân quyền truy cập theo vai trò và khoa/phòng
- Cơ chế fallback hoặc chuyển human review khi AI không đủ chắc chắn

5. Use case nào được đưa sang Phase 2?  
Trả lời đề xuất:
- Chatbot cho bệnh nhân hoặc người nhà bệnh nhân
- Tự động ghi ngược dữ liệu vào HIS/EMR
- Hỗ trợ quyết định chuyên môn hoặc gợi ý điều trị
- Dashboard phân tích nâng cao cho quản trị
- Tích hợp sâu với nhiều hệ thống cũ theo thời gian thực

6. Output nào hệ thống được phép trả về?  
Trả lời đề xuất:
- Bản tóm tắt hồ sơ bệnh án theo mẫu chuẩn
- Câu trả lời hành chính và quy trình nội bộ có trích nguồn
- Danh sách biểu mẫu, bước thực hiện, hướng dẫn tiếp theo
- Cảnh báo "không đủ chắc chắn" hoặc "cần xác nhận thủ công"
- Gợi ý chuyển tuyến sang người hoặc phòng ban phù hợp

7. Output nào hệ thống tuyệt đối không được trả về?  
Trả lời đề xuất:
- Chẩn đoán bệnh
- Khuyến nghị điều trị
- Kê đơn, liều dùng thuốc, y lệnh
- Kết luận chuyên môn không có xác nhận của con người
- Dữ liệu hồ sơ của người dùng không có quyền truy cập
- Nội dung bịa đặt khi không có nguồn chứng cứ

8. Hệ thống chỉ `read-only` hay có `write-back` vào HIS/EMR?  
Trả lời đề xuất: Trong MVP, hệ thống chỉ nên ở chế độ `read-only`. Không nên có `write-back` vào HIS/EMR ở giai đoạn đầu vì rủi ro vận hành, sai sót dữ liệu, và yêu cầu compliance cao hơn nhiều.

9. Tính năng nào là `must-have`, tính năng nào là `nice-to-have`?  
Trả lời đề xuất:
- `Must-have`: đăng nhập và phân quyền theo vai trò; tóm tắt hồ sơ ở chế độ `read-only`; hỏi đáp SOP nội bộ có trích nguồn; hỏi đáp hành chính; audit log cơ bản; fallback hoặc human review cho case rủi ro.
- `Nice-to-have`: streaming response; dashboard admin nâng cao; cache thông minh theo nhóm câu hỏi; đa ngôn ngữ; gợi ý câu hỏi mẫu theo vai trò; feedback analytics nâng cao.

10. Điều kiện nào khiến một request bị từ chối vì nằm ngoài scope?  
Trả lời đề xuất: Request phải bị từ chối nếu yêu cầu chẩn đoán hoặc khuyến nghị điều trị; yêu cầu kê đơn, chỉnh liều, ra y lệnh; yêu cầu truy cập dữ liệu ngoài quyền được cấp; yêu cầu sửa hoặc ghi ngược trực tiếp HIS/EMR; câu hỏi không có nguồn nội bộ đủ tin cậy để trả lời; hoặc có confidence thấp nhưng không đủ điều kiện tự động trả lời.
...
1. Chọn `cloud`, `on-prem`, hay `hybrid`?
2. Nếu hybrid, phần nào ở cloud và phần nào ở on-prem?
3. Region nào được phép đặt workload?
4. Database/vector store nào được chọn?
5. Có cần Kubernetes hay chỉ cần managed service là đủ?
6. Có cần queue không, nếu có thì dùng công nghệ nào?
7. Có cần GPU không, hay toàn bộ là API-based?
8. Hệ thống nào cần HA ngay từ MVP?
9. Backup, restore, DR mục tiêu là gì?
10. Metric nào dùng để autoscale?
11. Ba ràng buộc enterprise lớn nhất là gì?
12. Hai lý do mạnh nhất cho lựa chọn deployment cuối cùng là gì?
13. Dữ liệu nào được sử dụng trong MVP?
14. Dữ liệu nào nhạy cảm nhất, và mức độ nhạy cảm ra sao?

### 3.3 Bộ câu hỏi breakdown về lựa chọn cloud

1. Mục tiêu của hạ tầng là tối ưu bảo mật, chi phí, tốc độ triển khai hay khả năng scale?
2. Policy của bệnh viện có cho phép đưa PHI/PII lên cloud không?
3. Nếu không, phần nào bắt buộc phải giữ on-prem?
4. Nếu dùng cloud, cần private networking/VPN/connectivity nào?
5. AWS, Azure, GCP hay private cloud đang là lựa chọn thực tế?
6. Team hiện tại đã có sẵn kỹ năng trên nền tảng nào?
7. Nền tảng nào có pricing và managed service phù hợp nhất với MVP?
8. Nền tảng nào giúp compliance dễ hơn?
9. Lock-in của từng lựa chọn lớn đến đâu?
10. Lựa chọn nào giúp pilot nhanh nhất mà vẫn an toàn?

### 3.4 Bộ câu hỏi breakdown về sizing và capacity

1. DAU, request/day, peak QPS là bao nhiêu?
2. App service cần bao nhiêu CPU/RAM/replica cho baseline?
3. Vector store cần bao nhiêu dung lượng cho 3 tháng, 6 tháng, 12 tháng?
4. Queue worker cần bao nhiêu concurrency?
5. Log và monitoring retention bao lâu?
6. Egress/ingress giữa on-prem và cloud là bao nhiêu?
7. Nút cổ chai đầu tiên khi traffic tăng 5x sẽ là gì?
8. Nút cổ chai đầu tiên khi traffic tăng 10x sẽ là gì?
9. Có cần CDN/cache cho admin Q&A không?
10. Có cần warm standby hoặc multi-zone ngay từ MVP không?
...
| ------- | ------------ | ------------ | ----------------- | -------------------- | -------------- | ---------- |
| Cloud   | Thấp/Vừa/Cao | Thấp/Vừa/Cao | Thấp/Vừa/Cao      | Thấp/Vừa/Cao         | Thấp/Vừa/Cao   | Chọn/Không |
| On-prem | Thấp/Vừa/Cao | Thấp/Vừa/Cao | Thấp/Vừa/Cao      | Thấp/Vừa/Cao         | Thấp/Vừa/Cao   | Chọn/Không |
| Hybrid  | Thấp/Vừa/Cao | Thấp/Vừa/Cao | Thấp/Vừa/Cao      | Thấp/Vừa/Cao         | Thấp/Vừa/Cao   | Chọn/Không |
...
Cache + rule-based routing cho nhóm hỏi đáp hành chính lặp lại.
Model tiering + fallback policy theo độ phức tạp và độ rủi ro của use case.
Tối ưu retrieval + prompt budget để giảm token đầu vào và đầu ra.

1. Mỗi chiến lược tiết kiệm khoản chi phí nào?

Cache + routing tiết kiệm mạnh nhất ở Token/API; ngoài ra còn giảm compute và giảm áp lực support/human review.

Model tiering tiết kiệm chủ yếu ở Token/API, vì không phải request nào cũng đi vào model đắt.

Tối ưu retrieval/prompt tiết kiệm cả input token, output token, và giảm latency, từ đó giảm nhu cầu scale hạ tầng quá sớm.

1. Lợi ích lớn nhất của từng chiến lược là gì?

Cache + routing: hiệu quả nhanh, dễ thấy ngay trên nhóm admin Q&A, phù hợp MVP.

Model tiering: tối ưu chi phí theo đúng giá trị nghiệp vụ, không lãng phí model mạnh cho tác vụ đơn giản.

Tối ưu retrieval/prompt: giúp chi phí tăng chậm hơn khi user tăng, rất quan trọng ở giai đoạn scale.

1. Trade-off của từng chiến lược là gì?

Cache + routing: rủi ro trả lời theo dữ liệu cũ nếu tài liệu nội bộ thay đổi mà cache chưa được làm mới.

Model tiering: cần routing logic tốt; nếu route sai có thể làm giảm chất lượng hoặc tăng fallback.

Tối ưu retrieval/prompt: nếu giảm context quá tay thì câu trả lời có thể thiếu bằng chứng hoặc thiếu chi tiết.

1. Khi nào nên áp dụng từng chiến lược?

Cache + routing: áp dụng ngay khi xác định được top câu hỏi hành chính lặp lại.

Model tiering: áp dụng ngay sau khi chốt scope và phân nhóm use case theo rủi ro.

Tối ưu retrieval/prompt: áp dụng sau khi có dữ liệu log pilot để biết route nào đang tốn token nhất.

1. Chiến lược nào làm ngay?
2. Chiến lược nào làm sau?

### 5.2 Reliability & Risk Management
...
#### 5.2.2 Failure scenarios và tác động lên người dùng
...
#### 5.2.3 SLO, SLA và khả năng quan sát
...
#### 5.2.4 Incident response và vận hành sự cố
...
### 5.4 Proposal, slide và presentation
...
### 5.6 Nộp cuối ngày và Team Note
...
### 5.7 Số lượng user và chi phí tương ứng
